...
function Navbar() {
  ...
  return (
    <div className="w-full bg-richblack-800">
      <div className="w-11/12 max-w-maxContent flex bg-richblack-800 justify-between items-center mx-auto px-3 py-2">
        <div>
                ...
                <li key={index}>
                  {link.title === "Catalog" ? (
                    <div
                      ...
                      className="flex gap-2 relative items-center group"
                    >
                      <p className="text-richblack-50">{link.title}</p>
                      ...
                      <div className="absolute lg:w-[250px] z-[1000] invisible group-hover:visible hover:block hover:visible hover:opacity-100 bg-richblack-5 group-hover:opacity-100 transition-all duration-200 text-richblack-900 top-[80%] translate-y-2 -translate-x-[50%] px-2 rounded py-2">
                        ...
                        <div className="flex flex-col z-[1000]">
                          {subLinks.map((catagory, index) => (
                            <Link
                              to={`/catalouge/${catagory.name
                                .split(" ")
                                .join("-")
                                .toLowerCase()}`}
                              key={index}
                              className="hover:bg-richblack-200 px-3 py-2 rounded-md"
                            >
                              {catagory.name}
                            </Link>
                          ))}
        ...
        <div className="flex gap-4 justify-center items-center">
          ...
          {token === null && (
            <Link to={"/signUp"}>
              <button className="border cursor-pointer border-richblack-700 px-4 py-1 text-richblack-100 text-[14px] rounded-md hover:scale-95 hover:bg-yellow-50 hover:text-richblack-700 transition-all duration-200">
                Sign Up
              </button>
            </Link>
          )}
